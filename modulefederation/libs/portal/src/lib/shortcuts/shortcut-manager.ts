...
export class ShortcutManager {
    ...
    public register(shortcut : Shortcut) : (() => void) | undefined {
        if (Tracer.ENABLED) this.tracer.trace('views', TraceLevel.HIGH, 'register shortcut {0}', shortcut.shortcut);
    ...
    public pushLevel() : void {
        if (Tracer.ENABLED) this.tracer.trace('views', TraceLevel.HIGH, 'push shortcut level');
    ...
    public popLevel() : void {
        if (Tracer.ENABLED) this.tracer.trace('views', TraceLevel.HIGH, 'pop shortcut level');
    ...
    public handleKeydown(event : KeyboardEvent) : void {
        ...
        if (Tracer.ENABLED) this.tracer.trace('views', TraceLevel.HIGH, 'check for shortcut {0}', toString(key));
